...
CodeMirror.defineMode("irule", function() {
  ...
  "client_addr client_port clientside clone close connect cpu crc32 decode_uri discard drop event findclass " +
  "forward getfield htonl htons http_cookie http_header http_host http_method http_uri http_version " +
  "ifile imid ip_protocol ip_tos ip_ttl link_qos listen llookup local_addr log matchclass matches_glob " +
  "md5 members nexthop node nodes ntohl ntohs peer persist pool priority rateclass " +
  "recv redirect reject relate_client relate_server remote_addr send server_addr server_port serverside " +
  "session sha1 sha256 sha384 sha512 sharedvar snat snatpool substr table tcl_platform " +
  "traffic_group translate use virtual vlan_id");
  ...
  var functions = parseWords("if elseif else eq ne in ni for foreach while switch " +
    "contains ends_with equals matches matches_regex starts_with and not or");
